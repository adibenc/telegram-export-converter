...
class Message:
	...
	def toTuple(self):
		...
		if self.link: self.content = unescape(self.link.strip())
		if self.content: self.content = unescape(self.content.strip())

		return (
			self.message_id, self.timestamp, self.sender, self.fwd, self.reply, self.link, self.content
		)
...
html_link_gr = re.compile('<a.*href="(.*)">.*</a>')
...
lm = len(message_files)
#print(f"Loading all {lm} message files")
...
while cur < len(lines):
	...
	rem = re.search('<a.*href="(.*)">.*</a>', m.content)
	if rem:
		m.link = rem.group(1)
